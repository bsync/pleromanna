...
import sys; sys.path.append(pathlib.Path.home())
try:
    import secrets
...
#        resp = self.fetch('/me/albums', token) 

    def fetch(self, aurl, fparams=None, headers={}, token=None):
        headers.update(Host='api.vimeo.com')
        if (token): self.token = token
        if (self.token): 
            headers.update(Authorization='bearer {}'.format(self.token))
            
        return request.get('https://api.vimeo.com' + aurl, params=fparams, headers=ah).json()
...
client = VimeoClientClient(token=VIMEO_TOKEN)
